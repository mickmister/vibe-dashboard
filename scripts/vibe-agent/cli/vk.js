...
function boolFlag(flags, key, defaultValue) {
    const value = flags[key];
    if (value === undefined)
        return defaultValue;
    if (value === true)
...
function printProcess(proc) {
    ...
    if (action.working_dir)
        console.log(`Working Dir: ${action.working_dir}`);
    if (action.script)
        console.log(`Script:      ${action.script}`);
}
async function commandWorkspace(positional, flags) {
...
async function findRepoAndRemoteForPrUrl(prUrl, requestedRemote) {
    if (!prUrl.slug) {
    ...
    for (const repo of repos) {
        ...
        for (const remote of remotes) {
            if (requestedRemote && remote.name !== requestedRemote)
                continue;
...
async function commandWorkspaceCreateFromPr(positional, flags) {
    ...
    let repoId = repoFlag ?? nonUrlPositionals[0];
    let remoteName = remoteFlag ?? nonUrlPositionals[1];
...
async function commandCreateWorkspace(positional, flags) {
...
async function commandRepos(flags) {
...
async function commandRepo(repoId, flags) {
    if (!repoId) {
...
async function commandWorkspaceRepos(workspaceId, flags) {
    if (!workspaceId) {
...
async function commandDevScript(positional, flags) {
    const subcommand = positional[0];
...
async function commandDevServer(positional, flags) {
    const subcommand = positional[0];
    switch (subcommand) {
        case 'start': {
...
async function commandSessions(workspaceId) {
    if (!workspaceId) {
...
async function commandStatus(workspaceIds) {
...
async function commandProcesses(sessionId) {
    if (!sessionId) {
...
async function commandFetch(processId, flags) {
    if (!processId) {
...
async function commandCreateSession(workspaceId, executor) {
...
async function commandSend(sessionId, prompt) {
    if (!sessionId || !prompt) {
