...
class WebSocket {
  ...
  constructor(url: string) {
    if (!globalThis.WebSocket) {
      throw new Error(
        "Global WebSocket is unavailable in this Node.js runtime",
      );
    }
  ...
  on(
    ...
    handler:
      WebSocketMessageHandler | WebSocketErrorHandler | WebSocketCloseHandler,
  ): this {
    if (event === "message") {
      this.socket.addEventListener("message", (messageEvent) => {
        (handler as WebSocketMessageHandler)(
          normalizeWebSocketMessageData(messageEvent.data),
        );
      });
      return this;
    }
    if (event === "error") {
      this.socket.addEventListener("error", (errorEvent) => {
        ...
        (handler as WebSocketErrorHandler)(
...
export class VKService {
    ...
    const processes = (
      await Promise.all(
        sessions.map(async (session) => {
          ...
          return sessionProcesses.map((process) => ({
            ...process,
  ...
  private async parseApiResponse<T>(
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to ${action}: ${text.trim() || `HTTP ${response.status}`}`,
      );
    }
    ...
    if (!data.success) {
      const detail =
        data.message ?? JSON.stringify(data.error_data) ?? "Unknown error";
  ...
  async listRepos(options?: { recent?: boolean }): Promise<Repo[]> {
    const response = await fetch(
      options?.recent ? config.endpoints.recentRepos : config.endpoints.repos,
    );
  ...
  async getWorkspaceRepos(
    ...
    const response = await fetch(
      config.endpoints.taskAttemptRepos(workspaceId),
    );
    return this.parseApiResponse<RepoWithTargetBranch[]>(
      response,
  ...
  async listOpenPrs(
    ...
    const response = await fetch(
      config.endpoints.repoPullRequests(repoId, remoteName),
    );
    return this.parseApiResponse<PullRequestDetail[]>(
      response,
  ...
  async createWorkspaceFromPr(
    ...
    return this.parseApiResponse<CreateWorkspaceFromPrResponse>(
      response,
  ...
  async startWorkspace(
    ...
    const data = await this.parseApiResponse<Workspace | StartWorkspaceResult>(
      response,
      "start workspace",
    );
  ...
  async getWorkspaceSummaries(
    ...
    try {
      ...
      }>(response, "fetch workspace summaries");
      ...
      const summaries = await Promise.all(
        workspaceIds.map((id) => this.buildWorkspaceSummary(id)),
      );
      return summaries.filter(
        (summary): summary is WorkspaceSummary => summary !== null,
      );
    }
  }
  ...
  async sendMessage(
    ...
    if (!session.executor) {
      throw new Error(
        `Failed to send message: session ${sessionId} has no executor`,
      );
    }
  ...
  async startDevServer(workspaceId: string): Promise<ExecutionProcess[]> {
    const response = await fetch(config.endpoints.startDevServer(workspaceId), {
      method: "POST",
    });
    return this.parseApiResponse<ExecutionProcess[]>(
      response,
      "start dev server",
    );
  }
    ...
    const processes = (
      await Promise.all(
        sessions.map(async (session) => {
          ...
          return sessionProcesses.map((process) => ({
            ...process,
            session_id: process.session_id || session.id,
          }));
        }),
      )
    ).flat();
      ...
      ws.on("message", (data) => {
        ...
        if (msg.JsonPatch) {
          for (const op of msg.JsonPatch) {
            ...
            if (
              op.op === "remove" &&
              op.path.startsWith("/execution_processes/")
            ) {
  ...
  > {
    ...
    for (const session of sessions) {
      ...
      let latestUserMessage: string | null =
        this.getPromptFromProcess(latestProcess);
