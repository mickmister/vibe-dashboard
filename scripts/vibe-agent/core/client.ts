...
export class VibeClient {
      ...
      ws.on("message", (data) => {
        ...
        if (msg.JsonPatch) {
          for (const op of msg.JsonPatch) {
            ...
            if (
              op.op === "remove" &&
              String(op.path ?? "").startsWith("/execution_processes/")
            ) {
  ...
  async getWorkspaces(taskId: string): Promise<Workspace[]> {
    return this.request<Workspace[]>(
      config.endpoints.taskAttemptsByTask(taskId),
    );
  }
  ...
  async getWorkspaceSummary(
    ...
    try {
      ...
      const summaries: Array<WorkspaceSummary | null> = await Promise.all(
          ...
          const processes = (
            await Promise.all(
              sessions.map(async (session) => {
                const sessionProcesses = await this.getSessionProcesses(
                  session.id,
                );
  ...
  async sendMessage(
    ...
    return this.request<ExecutionProcess>(
      config.endpoints.sessionFollowUp(sessionId),
      {
        method: "POST",
        body: JSON.stringify(body),
      },
  ...
  async queueMessage(
    ...
    return this.request<QueueMessageResponse>(
      config.endpoints.sessionQueue(sessionId),
      {
        method: "POST",
        body: JSON.stringify(body),
      },
  ...
  async getExecutionProcess(processId: string): Promise<ExecutionProcess> {
    return this.request<ExecutionProcess>(
      config.endpoints.executionProcess(processId),
    );
  }
  ...
  async fetchConversation(
    ...
    return new Promise((resolve, reject) => {
      ...
      const applyPatch = (op: any) => {
        const pathParts = String(op.path ?? "")
          .split("/")
          .filter(Boolean);
      ...
      const timeout = setTimeout(() => {
        ...
        reject(
          new Error(
            `Timed out waiting for process ${processId} after ${timeoutMs}ms`,
          ),
        );
      }, timeoutMs);
