...
function fakeClient(input: {
  ...
  const client: NudgeDaemonClient = {
    ...
    async fetchConversation(processId: string) {
      return (
        input.entries?.[processId] ?? [
          entry("tool_use", "git status"),
          entry("thinking", "still working"),
        ]
      );
    },
...
describe("nudge daemon", () => {
  it("keeps the daemon disabled by default and allows explicit opt-in", () => {
    ...
    expect(isNudgeDaemonEnabled({ VD_NUDGE_DAEMON_ENABLED: "true" })).toBe(
      true,
    );
    expect(isNudgeDaemonEnabled({ VD_NUDGE_DAEMON_ENABLED: "false" })).toBe(
      false,
    );
  });
  ...
  it("only considers processes updated after the startup cutoff", () => {
    ...
    expect(
      processIsAfterStartupCutoff(
        process({
          created_at: "2026-06-26T09:50:00.000Z",
          started_at: "2026-06-26T09:50:00.000Z",
          updated_at: "2026-06-26T09:59:59.000Z",
          completed_at: "2026-06-26T09:59:59.000Z",
        }),
        cutoff,
      ),
    ).toBe(false);
    expect(
      processIsAfterStartupCutoff(
        process({ updated_at: "2026-06-26T10:00:00.000Z" }),
        cutoff,
      ),
    ).toBe(true);
    expect(
      processIsAfterStartupCutoff(
        process({ updated_at: "2026-06-26T10:00:01.000Z" }),
        cutoff,
      ),
    ).toBe(true);
  });
  ...
  it("sends a continue nudge for a new failed coding-agent process without a final response", async () => {
    ...
    const result = await runNudgeDaemonCycle(
      client,
      options(statePath),
      state,
      () => {},
    ...
    expect(readNudgeDaemonState(statePath).nudgedProcessIds).toEqual([
      "process-1",
    ]);
  });
  ...
  it("does not send a nudge for pre-startup failures", async () => {
    const { client, sent } = fakeClient({
      ...
      processes: [
        process({
          created_at: "2026-06-26T09:50:00.000Z",
          started_at: "2026-06-26T09:50:00.000Z",
          updated_at: "2026-06-26T09:55:00.000Z",
          completed_at: "2026-06-26T09:55:00.000Z",
        }),
      ],
    });
    ...
    const result = await runNudgeDaemonCycle(
      client,
      options(),
      readNudgeDaemonState(tempStatePath()),
      () => {},
    );
  ...
  it("does not nudge the same process twice after state is persisted", async () => {
    ...
    await runNudgeDaemonCycle(
      client,
      options(statePath),
      readNudgeDaemonState(statePath),
      () => {},
    );
    await runNudgeDaemonCycle(
      client,
