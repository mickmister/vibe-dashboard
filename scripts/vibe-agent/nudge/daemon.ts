...
export function readNudgeDaemonState(statePath: string): NudgeDaemonState {
  try {
    const parsed = JSON.parse(fs.readFileSync(statePath, 'utf8')) as Partial<NudgeDaemonState>;
    if (parsed.version === 1 && Array.isArray(parsed.nudgedProcessIds)) {
      return { version: 1, nudgedProcessIds: parsed.nudgedProcessIds.filter((id): id is string => typeof id === 'string') };
    }
  } catch {
...
export function writeNudgeDaemonState(statePath: string, state: NudgeDaemonState): void {
...
export function getContainerStartedAt(now = new Date()): Date {
  try {
    ...
    const candidateMs = [stat.birthtimeMs, stat.ctimeMs, stat.mtimeMs]
      .filter((value) => Number.isFinite(value) && value > 0 && value <= now.getTime())
      .sort((a, b) => b - a)[0];
...
export function processLatestTimestamp(process: ExecutionProcess): Date | null {
  const timestamps = [process.updated_at, process.completed_at, process.started_at, process.created_at]
...
export function processIsAfterStartupCutoff(process: ExecutionProcess, startedAfter: Date): boolean {
  const latestTimestamp = processLatestTimestamp(process);
  return latestTimestamp != null && latestTimestamp.getTime() >= startedAfter.getTime();
}

export function createNudgeDaemonOptions(env: NodeJS.ProcessEnv = process.env, now = new Date()): NudgeDaemonOptions {
  return {
    statePath: env.VD_NUDGE_DAEMON_STATE_PATH || DEFAULT_STATE_PATH,
    startedAfter: env.VD_NUDGE_DAEMON_STARTED_AFTER ? new Date(env.VD_NUDGE_DAEMON_STARTED_AFTER) : getContainerStartedAt(now),
    pollIntervalMs: parsePositiveIntegerEnv(env.VD_NUDGE_DAEMON_POLL_MS, DEFAULT_POLL_INTERVAL_MS),
    maxSessionsPerWorkspace: parsePositiveIntegerEnv(env.VD_NUDGE_DAEMON_MAX_SESSIONS, DEFAULT_MAX_SESSIONS_PER_WORKSPACE),
    maxProcessesPerSession: parsePositiveIntegerEnv(env.VD_NUDGE_DAEMON_MAX_PROCESSES, DEFAULT_MAX_PROCESSES_PER_SESSION),
    conversationTimeoutMs: parsePositiveIntegerEnv(env.VD_NUDGE_DAEMON_CONVERSATION_TIMEOUT_MS, DEFAULT_CONVERSATION_TIMEOUT_MS),
    nudgePrompt: env.VD_NUDGE_DAEMON_PROMPT || DEFAULT_NUDGE_PROMPT,
    criteria: {
      enableActiveStaleNudge: parseBoolean(env.VD_NUDGE_DAEMON_ACTIVE_STALE_ENABLED),
      activeStaleAfterMs: parsePositiveIntegerEnv(env.VD_NUDGE_DAEMON_ACTIVE_STALE_MS, 10 * 60 * 1000),
    },
  };
}
...
export async function runNudgeDaemonCycle(
  ...
  const result: NudgeDaemonCycleResult = { inspectedWorkspaces: 0, inspectedSessions: 0, nudgesSent: 0, errors: [] };
  const workspaces = await daemonClient.getAllWorkspaces();

  for (const workspace of workspaces.filter(workspace => !workspace.archived)) {
    result.inspectedWorkspaces += 1;
    let sessions: Session[];
    try {
      sessions = (await daemonClient.getSessions(workspace.id))
        .filter(session => new Date(session.updated_at).getTime() >= options.startedAfter.getTime())
        .sort((a, b) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime())
        .slice(0, options.maxSessionsPerWorkspace);
    } catch (err) {
      result.errors.push(`workspace ${workspace.id}: ${(err as Error).message}`);
      continue;
    }
    ...
    for (const session of sessions) {
      ...
      try {
        const processes = (await daemonClient.getSessionProcesses(session.id))
          .filter(process => processIsAfterStartupCutoff(process, options.startedAfter))
        ...
        for (const process of processes) {
          entriesByProcessId.set(process.id, await daemonClient.fetchConversation(process.id, options.conversationTimeoutMs));
        }
        ...
        result.nudgesSent += 1;
        log(`Sent nudge to session ${session.id} for process ${candidate.process.id}: ${candidate.decision.reason}`);
      } catch (err) {
...
export async function runNudgeDaemon(
  ...
  console.log(`vibe-agent nudge daemon started; considering processes updated at or after ${options.startedAfter.toISOString()}`);
  ...
  while (true) {
    try {
      ...
      if (result.nudgesSent > 0 || result.errors.length > 0) {
        console.log(JSON.stringify({ type: 'nudge-daemon-cycle', ...result, at: new Date().toISOString() }));
      }
    } catch (err) {
...
async function main(): Promise<void> {
  ...
  if (Number.isNaN(options.startedAfter.getTime())) {
    console.error('Invalid VD_NUDGE_DAEMON_STARTED_AFTER; expected an ISO timestamp');
    process.exit(1);
  }
