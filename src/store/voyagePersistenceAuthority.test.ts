...
describe('normalized Voyage startup authority', () => {
  ...
  it('uses the real default bootstrap authority for current workspace and installed plugin targets', async () => {
    ...
    publishProductionPanelTargetRouterAuthority({ builtInRoutes: {}, redirectGuards: {}, deliveryRoutes: { workspacePrefix: '/internal/panel-target/workspaces' }, createGuardOwner: (isCurrent) => ({ isCurrent, issueWorkspace: (kind, workspaceId, origin) => ({ location: `https://vk.test/workspaces/${workspaceId}${kind === 'code' ? '/vscode' : ''}`, guard: { deliveryUrl: `${origin}/internal/panel-target/workspaces/${workspaceId}/${kind}`, upstreamOrigin: 'https://vk.test' } }), issuePreview: async () => null }) });
