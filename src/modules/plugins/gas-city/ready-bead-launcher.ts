...
export class GasCityReadyBeadLauncher {
  ...
  async launchReady(input: ReadyBeadLaunchInput): Promise<ReadyBeadLaunchResult> {
    ...
    const lockKey = readyBeadLaunchLockKey(workspaceId, convoyId);
    ...
    return this.deps.lock.withLock(lockKey, async () => {
      ...
      for (const bead of ready) {
        if (convoyId && !bead.convoyIds.includes(convoyId) && bead.parentId !== convoyId) {
...
export class BdMetadataLiveSourceWorkflowReader {
  ...
  async listLiveSourceWorkflowBeadIds(input: {
    ...
    for (const key of ["workflow_id", "gc.workflow_id", "gc.root_bead_id"]) {
...
export function readyBeadLaunchLockKey(
  ...
  return convoyId?.trim()
    ? `vd.ready-bead-launcher.workspace.${workspaceId}.convoy.${convoyId.trim()}`
    : `vd.ready-bead-launcher.workspace.${workspaceId}`;
}
...
export function hasLiveGasCitySourceWorkflow(
  ...
  return Boolean(
    cleanOptional(bead.metadata["workflow_id"]) ??
      cleanOptional(bead.metadata["gc.workflow_id"]) ??
      cleanOptional(bead.metadata["gc.root_bead_id"]),
  );
}
...
function isSourceWorkflowConflict(message: string): boolean {
  ...
  return lower.includes("source workflow") && (lower.includes("already") || lower.includes("live") || lower.includes("conflict"));
}
