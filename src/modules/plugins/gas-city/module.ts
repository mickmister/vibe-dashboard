...
springboard.registerModule('plugin-gas-city', { rpcMode: 'remote' }, async (moduleAPI) => {
  const pluginRegistry = moduleAPI.getModule('plugin-registry');
  if (pluginRegistry) {
    await pluginRegistry.actions.registerContributions(contributions);
  }

  const dashboard = await moduleAPI.statesAPI.createPersistentState<GasCityDashboardState>(
    'plugin-gas-city',
    createDefaultGasCityDashboardState(),
  );
  ...
  const runGc = async (
    ...
    const childProcess = await importNode<typeof import('node:child_process')>(
      'node:child_process',
    );
      ...
      childProcess.execFile(
        ...
        (error, stdout, stderr) => {
          if (error) {
            ...
            reject(new Error(message || `Failed running ${gcBinary} ${args.join(' ')}`));
            return;
          }
          resolve({
            stdout: stdout.trim(),
  ...
  const refreshSessionsInternal = async (): Promise<GasCitySessionInfo[]> => {
    const { stdout } = await runGc(['session', 'list', '--json', '--state', 'all']);
    const sessions = (JSON.parse(stdout || '[]') as GasCitySessionInfo[]).sort(
      (a, b) => {
  ...
  const actions = moduleAPI.createActions({
    ...
    createSession: async (args: {
      ...
      withLoading(async () => {
        const command = ['session', 'new', args.template.trim(), '--no-attach'];
        if (args.alias?.trim()) {
    ...
    suspendSession: async (args: { sessionId: string }) =>
      withLoading(async () => {
        const stdout = await runAndStoreOutput(['session', 'suspend', args.sessionId]);
        await refreshSessionsInternal();
        return stdout;
      }),
    wakeSession: async (args: { sessionId: string }) =>
      withLoading(async () => {
        const stdout = await runAndStoreOutput(['session', 'wake', args.sessionId]);
        await refreshSessionsInternal();
        return stdout;
      }),
    killSession: async (args: { sessionId: string }) =>
      withLoading(async () => {
        const stdout = await runAndStoreOutput(['session', 'kill', args.sessionId]);
        await refreshSessionsInternal();
        return stdout;
      }),
