...
describe("WorkspaceWorkflowsHomeView", () => {
  it("renders a clean workspace workflows shell without debug terms", () => {
    ...
    expect(html).toContain(
      "Running now. Open the run page to see who has the next step.",
    );
  ...
  it("TEST_CASE_M104_1A renders launch summary, run-scoped instructions context, and session choices", () => {
    const html = renderToStaticMarkup(
      React.createElement(LaunchSummary, {
        ...
        selectedSessions: [
          {
            role: { id: "dev", label: "Dev" },
            text: "Create or reuse “Dev”",
            executorSummary: "Claude Code · recommended",
            warning: null,
          },
        ],
      }),
  ...
  return {
    ...
    userWorkflows: [
      ...
      },
      {
        ...
        status: "unavailable",
        version: null,
        unavailableReason: "Publish this workflow before running it.",
        canRun: false,
        inputs: [],
        roles: [],
      },
    ...
    recentRuns: [
      ...
      },
      {
        ...
        startedAt: 2,
        updatedAt: 3,
        detailUrl: "/dashboard/workflows/run-blocked",
      },
      {
        runId: "run-complete",
        ...
        startedAt: 1,
        updatedAt: 4,
        detailUrl: "/dashboard/workflows/run-complete",
      },
    ],
    recentBatches: [
      {
    ...
    needsInput: [
      {
        attentionItemId: "attention-a",
        title: "Answer planning questions",
        description: "Please fill out the form.",
        workflowName: "Feature workflow run",
        createdAt: 3,
        detailUrl: "/dashboard/workflows/legacy-attention",
      },
    ],
  };
}
