...
function roleTurnQueueOptions(input:Parameters<NativeGasCityRuntime['ensureRoleTurn']>[0]){return{source:'workflow' as const,operationKey:`native-turn:${input.operationKey}`,priority:60,provenance:{kind:'workflow' as const,label:'Native workflow role turn',workflow_run_id:input.operationKey,workflow_role_id:input.roleId},executorConfig:input.executor?{executor:input.executor as Executor,model_id:input.model,reasoning_id:input.reasoningId}:undefined};}
function assertQueuedRoleTurn(item:QueuedMessage,sessionId:string,input:Parameters<NativeGasCityRuntime['ensureRoleTurn']>[0]){const expected=roleTurnQueueOptions(input);if(item.status==='failed'||item.status==='cancelled')throw new QueueIdentityError('The durable role turn is not in an acceptable state.');const normalizeConfig=(value:any)=>value?{executor:value.executor,variant:value.variant??null,model_id:value.model_id??null,agent_id:value.agent_id??null,reasoning_id:value.reasoning_id??null,permission_policy:value.permission_policy??null}:null;const normalizeProvenance=(value:any)=>value?{kind:value.kind,label:value.label,workflow_run_id:value.workflow_run_id??null,workflow_name:value.workflow_name??null,workflow_design_id:value.workflow_design_id??null,workflow_version:value.workflow_version==null?null:Number(value.workflow_version)}:null;const actual={sessionId:item.session_id,workspaceId:item.workspace_id,operationKey:item.data.operation_key??null,message:item.data.message,executorConfig:normalizeConfig(item.data.executor_config),provenance:normalizeProvenance(item.data.provenance),source:item.source,sessionCommand:item.data.session_command??null,priority:Number(item.priority)};const wanted={sessionId,workspaceId:input.workspaceId,operationKey:expected.operationKey,message:input.prompt,executorConfig:normalizeConfig(expected.executorConfig),provenance:normalizeProvenance(expected.provenance),source:expected.source,sessionCommand:null,priority:expected.priority};if(JSON.stringify(actual)!==JSON.stringify(wanted))throw new QueueIdentityError('The durable role turn identity does not match the confirmed workflow.');}
