...
const createWorkspaceModule = async (moduleAPI: ModuleAPI) => {
  ...
  const actions = moduleAPI.createActions({
    ...
    openBeadSplit: async (args: {
      ...
      const tabGroup = workspaceState
        .getState()
        .tabGroups.find((candidate) => candidate.id === args.tabGroupId);
      if (!tabGroup?.workspace?.workspaceId) return undefined;
      if (args.agentTabId !== BUILT_IN_AGENT_TAB_ID) return undefined;

      return {
        tabGroupId: args.tabGroupId,
        pairId: BUILT_IN_AGENT_BEADS_PAIR_ID,
        beadsTabId: BUILT_IN_BEADS_TAB_ID,
      };
    },
    ...
    activateSavedVoyageEntry: async (args: {
      ...
      savedSessionsState.setState((current) => {
        ...
        const activeSpaceId =
          workspace.spaces.find((space) =>
            space.tabGroupIds.includes(entry.tabGroupId),
    ...
    removeVoyageEntryFromSavedSession: async (args: {
      ...
      savedSessionsState.setState((current) => {
        ...
        if (
          nextEntries.length === target.voyageEntries.length ||
          !nextEntries.length
        ) {
          return createSavedWorkspaceSessionState(sessions);
        }
        ...
        const fallbackEntry =
          nextEntries.find(
            (entry) => entry.id === target.activeVoyageEntryId,
          ) || nextEntries[0]!;
