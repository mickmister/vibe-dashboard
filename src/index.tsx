...
springboard.registerModule(
  ...
  async (moduleAPI) => {
      ...
      addVKWorkspace: async (args: {
        ...
        workspaceState.setStateImmer((draft) => {
          ...
          draft.tabGroups.push({
            id: tabGroupId,
            label:
              trimmedLabel.length > 30
                ? `${trimmedLabel.slice(0, 27)}...`
                : trimmedLabel || "Workspace",
            createdAt: new Date().toISOString(),
            tabs: [
              {
                id: agentTab,
    ...
    const WorkspaceRoute = () => {
      ...
      const gasCityPlugin = moduleAPI.getModule("plugin-gas-city") as
        | GasCityPluginModule
        | undefined;
      const pluginRegistryState = pluginRegistry.states.registry.useState();
      const gasCityState = gasCityPlugin?.states.dashboard.useState() as
        | GasCityDashboardState
        | undefined;
  ...
  interface AllModules {
    workspace: {
      ...
      actions: {
        ...
        deleteSpace: (args: {
          spaceId: string;
        }) => Promise<{ wasDeleted: boolean; deletedSpaceId: string }>;
        ...
        deletePair: (args: {
          tabGroupId: string;
          pairId: string;
        }) => Promise<{ firstTabId?: string; tabGroupId: string }>;
        addVKWorkspace: (args: {
          workspaceId: string;
          name: string;
          containerRef: string;
