...
describe("registerWorkflowRoutes", () => {
  ...
  it("TEST_CASE_M108_1C exposes prompt and skill picker assets with source/version metadata", async () => {
    ...
    await designStore.createRoleTemplate({
      ...
      skillRefs: [{ kind: "skill", id: "skill.testing.notes", version: 2, versionMode: "pinned" }],
    ...
    await expect(response.json()).resolves.toMatchObject({
      ...
      roleTemplates: [
        expect.objectContaining({
          ...
          promptRefs: [expect.objectContaining({ kind: "prompt", id: "prompt.dev.instructions", versionMode: "latest" })],
          skillRefs: [expect.objectContaining({ kind: "skill", id: "skill.testing.notes", version: 2, versionMode: "pinned" })],
          executorPreference: { executorType: "CODEX", model: "gpt-5-codex", mode: "preferred" },
        }),
      ],
    });
