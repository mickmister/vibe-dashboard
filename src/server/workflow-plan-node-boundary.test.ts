...
describe("workflow plan Node server authorization boundary", () => {
  ...
  it("proves route-level launch CAS, conflicts, and mandatory stale reconciliation", async () => {
    ...
    const service = new WorkflowPlanLaunchService({ source: source as any, compiler: compiler as any, launcher, store: new DbWorkflowPlanStore({ getDb: async () => handle.db, now: () => now, leaseMs: 10, ownerId: "route-test" }), now: () => now, ttlMs: 10_000 });
    const authOptions: { browserOrigin?: string; now: () => number } = { now: () => now };
    const app = new Hono(); registerWorkflowRoutes(app, { registry: createWorkflowRegistry(), workflowPlanLaunchService: service, workflowPlanAuthService: new WorkflowPlanAuthService(authOptions) });
