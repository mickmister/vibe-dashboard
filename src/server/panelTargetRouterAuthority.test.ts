...
describe('router and redirect-guard Panel authority owner', () => {
  it('publishes an explicit ready-empty production registry', () => {
    ...
    owner.publish({ builtInRoutes: {}, redirectGuards: {}, deliveryRoutes: { workspacePrefix: '/w', previewPrefix: '/p' }, createGuardOwner: (isCurrent) => ({ isCurrent, issueWorkspace: () => null, issuePreview: async () => null }) });
