...
export function getIframeLayerTabsForPanel(
  ...
  visibleIframeTabs: RetainedIframeTab[],
...
export function IframePanel({
  ...
  const visibleIframeKeys = retainedVisibleTabIds
    ? new Set(
        (allKnownIframeTabs || visibleRetainedIframeTabs)
          .filter((item) => retainedVisibleTabIds.has(item.tab.id))
          .map((item) => item.iframeKey),
      )
    : new Set(visibleRetainedIframeTabs.map((item) => item.iframeKey));
  const retainedTabs =
