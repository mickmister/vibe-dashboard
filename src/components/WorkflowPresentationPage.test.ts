...
describe("WorkflowPresentationView", () => {
  it("renders the clean story timeline without debug vocabulary", () => {
    const html = renderToStaticMarkup(
      React.createElement(WorkflowPresentationView, {
...
function presentationFixture(): WorkflowPresentationModel {
  return {
    ...
    summary: {
      ...
      waitingReason: "Waiting for reviewer response.",
      nextAction: "The workflow resumes when the agent turn completes.",
    },
    callTree: [
      {
        turnId: "call-child",
    ...
    attention: null,
    provenance: {
      label: "Two agent review round workflow v2",
      workflowName: "Two agent review round",
      workflowDesignId: "two-agent-review-round",
      workflowVersion: 2,
    },
    timeline: [
      {
        ...
        status: "Complete",
        session: {
          label: "Implementer session",
          workspaceId: "ws-1",
          sessionId: "session-1",
        },
        initialMessage: {
          text: "Please implement the page.",
          truncated: false,
          maxChars: 4096,
        },
        finalResponse: {
          text: "Implemented the UI.",
          truncated: false,
          maxChars: 20_000,
        },
        responseUnavailable: null,
        commits: [
          { before: "aaaaaaaaaaaa", after: "bbbbbbbbbbbb", merge: null },
        ],
      },
      {
        id: "reviewer",
