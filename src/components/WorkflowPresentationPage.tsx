...
export function WorkflowPresentationById({ instanceId }: { instanceId: string }): React.ReactElement {
  const [presentation, setPresentation] = useState<WorkflowPresentationModel | null>(null);
  ...
  return <WorkflowPresentationView presentation={presentation} error={error} loading={loading} onRefresh={() => void load()} />;
}

export function WorkflowPresentationView({ presentation, error, loading, onRefresh }: {
    ...
    <StandaloneDashboardPage contentClassName="mx-auto max-w-5xl space-y-5">
        <header className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <div className="text-xs uppercase tracking-wide text-cyan-300">Workflow</div>
              <h1 className="mt-1 text-2xl font-semibold">{presentation.workflowName}</h1>
              <div className="mt-2 flex flex-wrap gap-2 text-xs">
                <StatusPill label={statusLabel(presentation.status)} tone={statusTone(presentation.status)} />
                <StatusPill label={humanStatusLabel(presentation.humanStatus)} tone={presentation.humanStatus === 'waiting_for_user' ? 'amber' : 'zinc'} />
              </div>
            </div>
          ...
          {presentation.originalTask ? (
            <section className="mt-5 rounded-lg border border-zinc-800 bg-zinc-950 p-4">
              ...
              <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-zinc-100">{presentation.originalTask}</p>
            </section>
          ) : null}
          {error ? <div role="alert" className="mt-4 rounded border border-amber-900 bg-amber-950/30 p-3 text-sm text-amber-100">{error}</div> : null}
        </header>
        ...
        {presentation.attention?.status === 'active' ? (
          <section className="rounded-xl border border-amber-900 bg-amber-950/30 p-5">
            ...
            <h2 className="mt-1 text-lg font-semibold">{presentation.attention.title}</h2>
            {presentation.attention.description ? <p className="mt-2 whitespace-pre-wrap text-sm text-amber-50">{presentation.attention.description}</p> : null}
            {presentation.attention.formRef ? <div className="mt-3 text-sm text-amber-100">Open the linked form to continue.</div> : null}
          </section>
        ) : null}
    ...
    <article className="rounded-lg border border-zinc-800 bg-zinc-950 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {sessionHref ? <a className="rounded-md border border-cyan-900 px-2 py-1 text-xs text-cyan-200 hover:bg-cyan-950/40" href={sessionHref} target="_blank" rel="noreferrer">Open {item.role} session</a> : null}
        </div>
      </div>
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        <TextBlock title="Initial message" text={item.initialMessage} empty="Initial message unavailable." />
        <TextBlock title="Final response" text={item.finalResponse} empty={item.responseUnavailable ?? 'Final response unavailable.'} />
      </div>
      {item.commits.length ? (
        <div className="mt-4 rounded-md border border-zinc-800 bg-zinc-900/60 p-3">
          ...
          <ul className="mt-2 space-y-1 text-xs text-zinc-300">
            {item.commits.map((commit, index) => <li key={index}>{commit.before ?? 'start'} → {commit.after ?? commit.merge ?? 'latest'}{commit.merge ? ` · merge ${commit.merge}` : ''}</li>)}
          </ul>
        </div>
      ) : null}
    </article>
...
function TextBlock({ title, text, empty }: { title: string; text: { text: string; truncated: boolean; maxChars: number | null } | null; empty: string }) {
  return (
    <section className="rounded-md border border-zinc-800 bg-zinc-900/60 p-3">
      <div className="flex items-center justify-between gap-2">
        ...
        {text?.truncated ? <span className="rounded border border-amber-800 px-2 py-0.5 text-xs text-amber-200">Truncated</span> : null}
      </div>
      {text ? <p className="mt-2 max-h-72 overflow-auto whitespace-pre-wrap text-sm leading-6 text-zinc-100">{text.text}</p> : <p className="mt-2 text-sm text-zinc-400">{empty}</p>}
      {text?.truncated && text.maxChars ? <p className="mt-2 text-xs text-amber-200">Showing the first {text.maxChars} characters.</p> : null}
    </section>
  );
}

function StatusPill({ label, tone }: { label: string; tone: 'emerald' | 'cyan' | 'amber' | 'red' | 'zinc' }) {
  const classes = {
...
function humanStatusLabel(status: WorkflowPresentationModel['humanStatus']): string {
  switch (status) {
    case 'waiting_for_user': return 'Waiting for you';
    case 'resolved': return 'User answered';
    case 'cancelled': return 'No user action';
    default: return 'No user action';
  }
}
