...
type Observation = { closed: boolean; mode: string; ratio: number; pinned: boolean; transientMutations: number; transientMaximized: boolean; groups: number; active?: string; durableSnapshotUnchanged: boolean; durableMutations: number; toJSONCalls: number; fromJSONCalls: number; revisions: number; writes: number; history: number; payloads: number; iframeConnected: boolean; iframeWindowStable: boolean; focusRestored: boolean; durableMaximized: boolean };
...
test('TEST_CASE_M1_5C supplies transient native resize/maximize without Voyage mutation', async ({ page }) => {
  ...
  await invoke(page, 'maximize', 'left'); expect(await observe(page)).toMatchObject({ transientMaximized: true }); await invoke(page, 'restore');
  await invoke(page, 'maximize', 'right'); expect(await observe(page)).toMatchObject({ transientMaximized: true }); await invoke(page, 'restore');
...
test('TEST_CASE_M1_5C reconstructs trusted route intent and keeps durable maximize separate', async ({ page }) => {
  await page.goto('/spikes/dockview-contract/split-view.html?split=1&withCraft=../bad&withSurface=code&url=https://evil.test'); await expect(page.locator('#ready')).toHaveText('ready'); expect(await observe(page)).toMatchObject({ closed: true, groups: 0 });
  await page.goto('/spikes/dockview-contract/split-view.html?split=1&withCraft=craft-a&withSurface=code'); await expect(page.locator('#ready')).toHaveText('ready');
