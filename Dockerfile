...
# Build the VD-owned GC exec-provider bridge into a stable runtime binary.
RUN cd /opt/vibe-kanban-vscode-web-seed/packages/gc-session-vibe \
    && CGO_ENABLED=0 GOBIN=/usr/local/bin go install ./cmd/gc-session-vibe \
    && GC_EXEC_STATE_DIR=/tmp/gc-session-vibe-smoke gc-session-vibe list-running >/dev/null
