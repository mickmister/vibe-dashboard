...
const PLUGIN_SERVICE_RUNNER_COMMAND = '/usr/local/bin/vd-plugin-service-runner.mjs';
...
export function renderSupervisorProgramConfig(input: {
  ...
  return [
    ...
    `[program:${program}]`,
    `command=${PLUGIN_SERVICE_RUNNER_COMMAND}`,
    `directory=${directory}`,
...
export function renderServiceArgv(
  plugin: PluginServiceDefinition,
  service: SupervisorServiceDefinition,
  paths: PluginServiceOrchestratorPaths,
): string[] {
  return [
    expandTemplate(service.command, plugin, service, paths),
    ...(service.args ?? []).map((arg) => expandTemplate(arg, plugin, service, paths)),
  ];
}

function encodeServiceArgv(argv: string[]): string {
...
function renderSupervisorEnvironment(
  ...
  env.VD_SERVICE_ARGV_BASE64 = encodeServiceArgv(renderServiceArgv(plugin, service, paths));
...
function validateServiceDefinition(plugin: PluginServiceDefinition, service: SupervisorServiceDefinition): void {
  ...
  if ('preStart' in service) throw new Error(`Unsupported service preStart for ${plugin.id}/${service.id}; use structured installers or first-party runtime code instead`);
