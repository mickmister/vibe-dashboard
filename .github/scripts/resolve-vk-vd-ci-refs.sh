...
repository_dispatch_vd_branch="${REPOSITORY_DISPATCH_VD_BRANCH:-}"
...
resolve_vd() {
  ...
  case "$event_name" in
    ...
    repository_dispatch)
      ...
      elif [[ -n "$repository_dispatch_vd_branch" ]]; then
        vd_commit="$(remote_head_sha "$vd_repo_url" "$repository_dispatch_vd_branch")"
        if [[ -n "$vd_commit" ]]; then
          vd_branch="$repository_dispatch_vd_branch"
          vd_ref="$(head_ref "$vd_branch")"
          vd_resolution_source="repository_dispatch_requested_vd_branch"
        else
          notice "Repository dispatch requested VD branch '${repository_dispatch_vd_branch}', but it was not found; falling back to ${default_branch}."
        fi
      fi
...
write_outputs() {
  ...
  local branch_pairing="VD ${vd_branch} (${vd_resolution_source}) / VK ${vk_branch} (${vk_resolution_source})"

  {
    ...
    echo "vk_ref_source=$vk_resolution_source"
    echo "vd_branch=$vd_branch"
    echo "vd_ref_name=$vd_branch"
    echo "vd_ref=$vd_ref"
    echo "vd_commit=$vd_commit"
    echo "vd_short_commit=$vd_short_commit"
    echo "vd_resolution_source=$vd_resolution_source"
    echo "branch_pairing=$branch_pairing"
    echo "release_tag=$release_tag"
    echo "release_url=$release_url"
    echo "workflow_run_url=$vk_workflow_run_url"
    echo "publish_latest=$publish_latest"
  } >> "$output_file"

  if [[ -n "${GITHUB_STEP_SUMMARY:-}" ]]; then
    {
      echo "## VK/VD publish correlation"
      echo
      echo "| Field | Value |"
      echo "| --- | --- |"
      echo "| VD repository | \`${github_repository}\` |"
      echo "| VD ref | \`${vd_ref}\` |"
      echo "| VD branch | \`${vd_branch}\` |"
      echo "| VD SHA | \`${vd_commit}\` |"
      echo "| VD resolution | ${vd_resolution_source} |"
      echo "| VK repository | \`${vk_repo_url}\` |"
      echo "| VK requested ref | \`${vk_branch}\` |"
      echo "| VK resolved SHA | \`${vk_commit}\` |"
      echo "| VK resolution | ${vk_resolution_source} |"
      echo "| Branch pairing | ${branch_pairing} |"
      if [[ -n "$release_tag" ]]; then
        echo "| VK release tag | \`${release_tag}\` |"
      fi
      if [[ -n "$release_url" ]]; then
        echo "| VK release URL | ${release_url} |"
      fi
      if [[ -n "$vk_workflow_run_url" ]]; then
        echo "| VK workflow run | ${vk_workflow_run_url} |"
      fi
      echo
      echo "Image tags planned:"
      [[ -n "$image_tag_input" ]] && echo "- \`${image_tag_input}\`"
      echo "- \`${deploy_image_tag}\`"
      echo "- \`vk-${vk_short_commit}-vd-${vd_short_commit}\`"
