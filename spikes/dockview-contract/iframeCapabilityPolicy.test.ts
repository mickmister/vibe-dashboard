...
const registry: CapabilityRegistry = {
  ...
  definitions: {
    vd: { provenance: 'vd-built-in', resolvedUrl: 'https://vd.example.test/dashboard', requested: ['same-origin', 'forms', 'clipboard-read', 'clipboard-write', 'fullscreen'], redirectBoundary: { kind: 'guarded-proxy', deliveryUrl: 'https://vd.example.test/dashboard', upstreamOrigin: 'https://vd.example.test' } },
    vk: { provenance: 'vk-built-in', resolvedUrl: 'https://vk.example.test/workspaces/ws-1', requested: ['same-origin', 'forms', 'clipboard-read', 'clipboard-write', 'fullscreen'], redirectBoundary: { kind: 'guarded-proxy', deliveryUrl: 'https://vk.example.test/workspaces/ws-1', upstreamOrigin: 'https://vk.example.test' } },
  ...
  plugins: { notes: { version: '1.2.3', dedicatedOrigin: 'https://plugins.example.test', contributions: { editor: { allowSameOrigin: true } } } },
};
...
describe('resolver-derived iframe capability policy', () => {
  ...
  it('requires an enforceable redirect guard before ambient privileges', () => {
    const unguarded: CapabilityRegistry = { ...registry, definitions: { vk: { provenance: 'vk-built-in', resolvedUrl: 'https://vk.example.test/', requested: ['same-origin', 'clipboard-read'] } } };
...
describe('postMessage boundary', () => {
  it('requires exact origin, source, schema, Panel/runtime identity, and generation', () => {
    ...
    for (const event of [
      ...
      { origin: 'https://vk.example.test', source: sourceWindow, data: { ...data, payload: { protocolVersion: 1, extra: true } } },
      { origin: 'https://vk.example.test', source: sourceWindow, data: { ...data, type: 'runtime-state', payload: { visibility: 'active', heartbeat: -1 } } },
