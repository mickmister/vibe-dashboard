...
const CEILINGS: Record<Provenance, ReadonlySet<CapabilityName>> = {
  ...
  // No redirect-proof, version-bound, per-plugin origin is enforced by the
  // current server/Caddy routes, so plugins stay opaque despite manifest claims.
...
function derivePolicy(definition: TrustedCapabilityDefinition, baseOrigin: string): EffectiveIframePolicy | null {
  ...
  for (const request of definition.requested) if (ceiling.has(request)) granted.add(request);
...
export function validateRuntimeMessage(event: { origin: string; source: unknown; data: unknown }, registration: RuntimeRegistration):
  ...
  if (data.schemaVersion !== 1 || (data.type !== 'runtime-ready' && data.type !== 'runtime-state') || data.runtimeId !== registration.runtimeId || data.panelId !== registration.panelId || data.generation !== registration.generation || !isObject(data.payload)) return { ok: false, reason: 'invalid-schema' };
