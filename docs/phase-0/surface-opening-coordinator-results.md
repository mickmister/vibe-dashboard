...
- The literal visible-control sequence **Open maximized → Restore → Undo → Redo** is exercised in Chromium. Guarded restoration exits the current maximize presentation before `fromJSON`, reapplies the serialized active view, suppresses intermediate restore callbacks, and publishes status only after the atomic redo reaches revision 4/cursor 2 with the post-Restore topology.
