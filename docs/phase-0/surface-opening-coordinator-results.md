...
- Runtime payloads preserve boot identity through guarded canonical undo/redo and controller eviction. Durable MRU survives warm eviction and reload. The atomic boundary compares the complete canonical envelope to live Dockview; the visible projection check additionally covers group membership, tab order, active Panel per group, supported split orientation, and maximize state after commands, including commands after Restore/undo/redo.
- A deliberately corrupt canonical envelope is produced after a real Dockview mutation; rejection restores the prior envelope with the aggregate revision, history cursor, activation state, and runtime identity unchanged.
