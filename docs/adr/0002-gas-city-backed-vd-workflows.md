...
For Docker, `VD_WORKFLOW_LOCAL_ORIGIN` must be the exact direct VD listener origin visible to the browser, including the published port (for example, `http://127.0.0.1:43123`). It must not name Caddy or another reverse-proxy port. The default uses the direct `PORT` value; a remapped listener therefore requires an explicit override. Missing, non-loopback, host-spoofed, or proxy-derived configuration fails closed.
